...
function nodePosAtDOM(node: Element, view: EditorView, options: GlobalFrontHandleOptions) {
...
export function DragHandlePlugin(options: GlobalFrontHandleOptions & { pluginKey: string }) {
  ...
  function hideHandleOnEditorOut(event: MouseEvent) {
    if (event.target instanceof Element) {
      ...
      const isDragHandle = !!event.target.attributes.getNamedItem("data-drag-handle");
      const isFrontHandle = !!event.target.attributes.getNamedItem("data-front-handle");
  ...
  return new Plugin({
    ...
    view: (view) => {
      ...
      view?.dom?.parentElement?.parentElement?.addEventListener("mouseout", hideHandleOnEditorOut);
